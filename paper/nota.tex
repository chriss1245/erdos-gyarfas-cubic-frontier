...
required to reproduce thirteen reference censuses of cubic graphs, nine of
which have nonzero answers and two of which are compared as sets of graphs
rather than as counts. The ground truth includes a 348.7 core-hour exhaustive
...
third-party reproduction is outstanding. A deposit of 4~August 2026 by
Howe~\cite{howe2026} reports a computer-assisted proof of the order-30 cubic
case by a different route (stratification by triangle count with a cycle-lift
argument); the 348.7 core-hour exhaustive enumeration of order~30 used as
ground truth here was completed in July 2026, independently of that work.
Tranquilli~\cite{tranquilli2026} treats the cubic \emph{bipartite} subclass
and reports the bound $n\ge 60$ there; a deposit of 17~August 2026 by
Zhuge~\cite{zhuge2026} reports an extension of that subclass bound to
$n\ge 70$.
...
\midrule
$n=10$ & $C_{16}$ & 19 (no 16-cycle fits) & 19 \\
...
Two robustness axes were varied. First, re-deciding with the totalizer
cardinality encoding, a structurally different CNF for the same degree
constraint, reproduces $251$ at $n=28$, $1471$ at $n=18$ and zero at $n=30$,
$n=32$, $n=36$ and $n=40$; the last two are frontier orders, re-decided in
4143\,s and 17\,207\,s against 3366\,s and 18\,922\,s for the sequential
counter, so the largest order reported here has been decided twice with
structurally different formulas. Second, re-deciding $n=32$ with the default
minimality-check cutoff of $2\cdot10^5$, instead of the cutoff-free
configuration used throughout, also returns zero, in 607\,s against 505\,s for
the cutoff-free run; this axis is used only for unsatisfiable instances, since
a truncated minimality check may admit isomorphic copies into nonzero counts.
The comparison of graph sets at $n=24$ and $n=26$ is exact.
...
reproduces every reference census available in this class, including nine with
...
Repository:
\url{https://github.com/chriss1245/erdos-gyarfas-cubic-frontier};
...
\bibitem{howe2026} J.~D. Howe, The Erd\H{o}s--Gy\'arf\'as conjecture for cubic
  graphs of order 30, Zenodo (2026),
  \url{https://doi.org/10.5281/zenodo.21797776} (deposited 4 August 2026).
\bibitem{zhuge2026} Y.~Zhuge, A 70-vertex lower bound for cubic bipartite
  counterexamples to the Erd\H{o}s--Gy\'arf\'as conjecture, Zenodo (2026),
  \url{https://doi.org/10.5281/zenodo.21981786} (reproducibility deposit
  including the manuscript, 17 August 2026).
